...
def _compute_output_shape(input_size: Tuple[int, int], kernel_size: Tuple[int, int],
...
class MaxPool2dFunction(Function):
    ...
    @staticmethod
    def forward(ctx: Context, x: Tensor, kernel_size: Tuple[int, int],
                ...
                for c in range(C):
                    for h in range(H_out):
                        for w in range(W_out):
                            ...
                            w_start = w * stride[1]
                            w_end = w_start + kW
                            grad[n, c, h_start:h_end, w_start:w_end] += grad_output[n, c, h, w] * scale
