    ...
    def backward(ctx, grad_output: np.ndarray, grad_dict: Dict[int, np.ndarray]) -> None:
        ...
        if a.requires_grad:
            ...
            if len(grad_output.shape) == len(b.data.shape):
                ...
                grad_a = np.matmul(grad_output.reshape(-1, grad_output.shape[-1]), 
        ...
        if b.requires_grad:
            ...
            if len(grad_output.shape) == len(a.data.shape):
                ...
                # Reshape grad_output to match dimensions if necessary
                grad_b = np.matmul(a.data.reshape(-1, a.data.shape[-2]).T,
                                 grad_output.reshape(-1, grad_output.shape[-1]))
                grad_b = grad_b.reshape(a.data.shape[:-2] + (a.data.shape[-1], grad_output.shape[-1]))
...
class Softmax(Function):
    ...
    def backward(ctx, grad_output: np.ndarray, grad_dict: Dict[int, np.ndarray]) -> None:
        ...
        if x.requires_grad:
            ...
            grad_x = softmax_out.data * (grad_output - 
