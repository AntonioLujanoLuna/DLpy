...
class Conv2d(Module):
    ...
    def __init__(
        ...
        weight = Tensor(
            np.random.uniform(-bound, bound, weight_shape), requires_grad=True
        )
    ...
    def calc_output_shape(
        ...
        H_out = (H + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1) // stride[
            0
        ] + 1
        W_out = (W + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1) // stride[
            1
        ] + 1
