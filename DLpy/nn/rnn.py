    ...
    def forward(self, x: Tensor, hx: Optional[Tuple[Tensor, Tensor]] = None) -> Tuple[Tensor, Tuple[Tensor, Tensor]]:
        ...
        if hx is None:
            ...
            if h_0.shape != expected_shape or c_0.shape != expected_shape:
                raise ValueError(f"Expected hidden size {expected_shape}, got {h_0.shape} and {c_0.shape}")
        ...
        for layer in range(self.num_layers):
            ...
            if self.bidirectional:
                ...
                layer_output = Tensor(np.concatenate([layer_forward.data, layer_backward.data], axis=-1))
            else:
...
class LSTMCell(Module):
    ...
    def forward(self, x: Tensor, 
                hx: Optional[Tuple[Tensor, Tensor]] = None) -> Tuple[Tensor, Tensor]:
